...
| Команда | Що робить |
|---|---|
| `/gku:init` | Створює `CLAUDE.md` репозиторію — команди та домовленості його родини |
| `/gku:plan` | Перетворює запит або письмовий опис на план, звірений із реальним кодом |
| `/gku:implement` | Виконує його крок за кроком, продовжуючи з місця зупинки після перерви |
| `/gku:review` | Перевіряє ваші власні зміни перед відправкою |
| `/gku:pr-review` | Рецензує чужий пул-реквест в ізольованому робочому дереві |
| `/gku:pr-resolve` | Опрацьовує коментарі рецензентів — спершу вердикт, жодних сліпих виправлень |
| `/gku:verify` | Запускає тести й перевіряє, що воно справді працює |
...
3. **Використовуйте Docker для локальної розробки.** Більшість наших проєктів має
   ...
   потрібно.
4. **Перевірте власну роботу перед тим, як просити рецензію** — командою `/gku:review` з нашого
