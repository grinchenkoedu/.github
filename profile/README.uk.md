...
Обидва набори покривають те саме. Назви різняться, бо Claude Code додає префікс до навичок
плагіна, а Antigravity — ні:

| Що робить | Claude Code | Antigravity |
|---|---|---|
| Перетворює запит або письмовий опис на план, звірений із реальним кодом | `/gku:plan` | `/plan` |
| Виконує його крок за кроком, продовжуючи з місця зупинки після перерви | `/gku:implement` | `/implement` |
| Перевіряє ваші власні зміни перед відправкою | `/gku:review` | `/review` |
| Рецензує чужий пул-реквест в ізольованому робочому дереві | `/gku:pr-review` | `/pr-review` |
| Опрацьовує коментарі рецензентів — спершу вердикт, жодних сліпих виправлень | `/gku:pr-resolve` | `/pr-resolve` |
| Запускає тести й перевіряє, що воно справді працює | `/gku:verify` | `/verify` |
...
3. **Використовуйте Docker для локальної розробки.** Більшість наших проєктів має
   ...
   потрібно.
4. **Перевірте власну роботу перед тим, як просити рецензію** — командою `/gku:review` чи `/review` з наших
